...
import com.quicksolveplus.qspmobile.databinding.FragmentCovidVaccineOneBinding
...
class CovidVaccineFragmentOne : Fragment() {

    lateinit var binding: FragmentCovidVaccineOneBinding
    private val viewModel by lazy {
        ViewModelProvider(this)[CovidFormViewModel::class.java]
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = FragmentCovidVaccineOneBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        binding.apply {
            btnNext.setOnClickListener {
                if (binding.rbYes.isChecked) {
                    openCovidVaccineWantToUploadProofFragmentTwo()
                } else if (binding.rbNo.isChecked) {
                    openSelectVaccinePlanFragment()
                }
            }
    ...
    private fun openCovidVaccineWantToUploadProofFragmentTwo() {
        val fragmentManager = parentFragmentManager
        val uploadVaccineCertificateFragment: Fragment = UploadVaccineCertificateFragment().newInstance()
        val fragmentTransaction = fragmentManager.beginTransaction()
        fragmentTransaction.add(
            R.id.flDeclaration,
            uploadVaccineCertificateFragment,
            uploadVaccineCertificateFragment.javaClass.name
        )
        fragmentTransaction.addToBackStack(uploadVaccineCertificateFragment.javaClass.name)
        fragmentTransaction.commit()
    }

    private fun openSelectVaccinePlanFragment() {
        val fragmentManager = parentFragmentManager
        val selectVaccinePlanFragment: Fragment = SelectVaccinePlanFragment().newInstance()
        val fragmentTransaction = fragmentManager.beginTransaction()
        fragmentTransaction.add(R.id.flDeclaration, selectVaccinePlanFragment, selectVaccinePlanFragment.javaClass.name)
        fragmentTransaction.addToBackStack(selectVaccinePlanFragment.javaClass.name)
        fragmentTransaction.commit()
    }
